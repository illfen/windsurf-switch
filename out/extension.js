...
async function doSwitch(context, sidebar, account) {
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            statusOk(`已切到 ${session?.account?.label ?? account.email} · 浏览器登录页可直接关闭`);
...
async function probeCurrentQuota(context) {
    ...
    void sidebar?.reload();
    const dailyZero = typeof snap.dailyRemainPct === 'number' && snap.dailyRemainPct <= 0;
    const weeklyZero = typeof snap.weeklyRemainPct === 'number' && snap.weeklyRemainPct <= 0;
