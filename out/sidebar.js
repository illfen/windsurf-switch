    ...
    postState() {
        ...
        const payload = {
            ...
            ui: {
                ...
                autoCollapsed: gs.get(UI_KEYS.autoCollapsed, true)
            }
        };
...
<body>
    ...
    <div class="toolbar">
        <button class="btn-primary" data-cmd="addAccount" title="添加账号">
            <span class="ico">＋</span><span>添加</span>
        </button>
        <button class="btn" data-cmd="batchImport" title="批量导入">批量导入</button>
        <button class="btn" data-cmd="refreshAll" title="刷新全部 Plan / Quota">刷新全部</button>
    </div>

    <div class="section-label">当前账号</div>
    <div id="current-account"></div>

    <div class="section-head" data-collapse="auto">
        <span class="caret"></span>
        <span class="section-label">自动切号</span>
    </div>
    <div class="auto-block section-body" id="auto-block" data-body="auto">
        ...
        <div class="list-header-controls">
            ...
            <div class="dropdown" data-dd="filter">
                ...
                <div class="dropdown-menu" id="filter-menu" hidden>
                    ...
                    <label class="dropdown-check"><input type="checkbox" data-filter="exclude-no-quota"/><span>除无额账号</span></label>
...
/* ===========================================================================
 * Auto-switch — TOP-LEVEL section (peer of "当前账号"), holds child sub-sections
 * (排序方式 / 筛选账号) inside its body for grouping.
 * ========================================================================= */
.auto-block {
  display: flex;
  flex-direction: column;
  gap: var(--sp-1);
  padding: 2px 0 var(--sp-1) 0;
}
.auto-block.collapsed {
  display: none;
}
.auto-row {
  display: flex;
...
(function () {
    ...
    function applyCollapseUi() {
        // Top-level: 自动切号
        const autoHead = document.querySelector('.section-head[data-collapse="auto"]');
        const autoBody = document.querySelector('.section-body[data-body="auto"]');
        if (autoHead) autoHead.classList.toggle('collapsed', !!state.ui.autoCollapsed);
        if (autoBody) autoBody.classList.toggle('collapsed', !!state.ui.autoCollapsed);
        // Sub-level (inside 自动切号): 排序方式 / 筛选账号
...
<div class="auto-row">
  <label>
    ...
    <span>监控 Windsurf 日志（实验性）</span>
  </label>
  <span class="auto-hint" title="依赖 Windsurf 内部日志，升级后可能失效">ⓘ</span>
</div>\`;
    }
    ...
    // Section head collapse — top-level 自动切号 + sub-level 排序方式 / 筛选账号
    document.querySelectorAll('.section-head[data-collapse]').forEach(head => {
        head.addEventListener('click', () => {
            const section = head.dataset.collapse;
            if (section === 'auto') {
                state.ui.autoCollapsed = !state.ui.autoCollapsed;
                applyCollapseUi();
                post('toggleCollapse', { section, collapsed: state.ui.autoCollapsed });
